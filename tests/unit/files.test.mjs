...
test('accepts common written document formats and rejects unsafe names',()=>{for(const name of ['nota.txt','leitura.md','dados.csv','texto.PDF','carta.doc','relatorio.docx','arquivo.odt','nota.rtf','rascunho.pages'])assert.equal(validateFileMetadata(name,10).ok,true);for(const name of ['a.html','a.exe','../a.txt','a/b.txt','a\\b.md','a\0.txt','a\n.txt'])assert.equal(validateFileMetadata(name,10).ok,false)});
