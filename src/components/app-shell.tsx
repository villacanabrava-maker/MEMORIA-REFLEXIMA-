...
export async function AppShell({ active, title, eyebrow, children }: { active: "inicio" | "biblioteca" | "memoria" | "cerebro" | "reflexoes"; title: string; eyebrow: string; children: ReactNode }) {
    ...
    <div className="app-frame workspace">
      ...
      <aside className="sidebar">
        ...
        <nav className="workspace-nav" aria-label="Navegação principal">
          ...
          <Link href="/reflexoes" aria-current={active === "reflexoes" ? "page" : undefined} title="Reflexões"><span aria-hidden="true">✦</span><span className="nav-label">Reflexões</span></Link>
        </nav>
        <div className="sidebar-bottom"><div className="privacy"><span aria-hidden="true">⌾</span><div><strong>Seu espaço privado</strong><p>Originais, evidências, memórias, interpretações e reflexões com acesso por usuário.</p></div></div></div>
      </aside>
